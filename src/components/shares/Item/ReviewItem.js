    ...
    <View className='px-5 mb-3 radius-sm'>
        <View className='flex-row items-center justify-between'>
            <View className='flex-row items-center'>
                ...
                <Text style={[MyTheme.typography.medium.medium_2, {color:MyTheme.colors.black}]}>{review.name}</Text>
                <Text style={[MyTheme.typography.body.body_2, {color:MyTheme.colors.neutral_2p}]}>{review.date}</Text>
                </View>
            </View>
            <View style={styles.ratingContainer}>
                <Image source={require('../../../../assets/icons/star.png')} style={{height: 15, width: 15, marginLeft:8 }} />
                <Text style={{fontFamily: 'poppinsSemiBold', fontSize: 12, color: MyTheme.colors.black}} className='pl-0.5 pr-2 pt-0.5'>{review.score}</Text>
            </View>
        </View>
